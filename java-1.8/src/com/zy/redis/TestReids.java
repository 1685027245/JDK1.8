...
public class TestReids {
	...
	public void testString() {
		...
		// 设置多个键值对
		jedis.mset("name", "zy", "age", "22", "qq", "1933108196");
		jedis.incr("age");// 加1操作
		jedis.exists("name");//判断是否存在
		jedis.expire("name", 20);//设置过去时间,20s
		jedis.ttl("name");//查看某个key的剩余生存时间,单位【秒】.永久生存或者不存在的都返回-1
		//jedis.persist("name");//移除剩余生存时间
